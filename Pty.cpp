...
void Pty::sendData(const QByteArray &data)
{
    ...
    if (pty()->write(data) == -1) {
        qCDebug(TerminalDebug) << "Could not send input data to terminal process.";
        return;
    }
}
...
void Pty::setFlowControlEnabled(bool enable)
{
    ...
    if (pty()->masterFd() >= 0) {
        ...
        if (!pty()->tcSetAttr(&ttmode)) {
            qCDebug(TerminalDebug) << "Unable to set terminal attributes.";
        }
    }
}
...
bool Pty::flowControlEnabled() const
{
    if (pty()->masterFd() >= 0) {
        ...
        qCDebug(TerminalDebug) << "Unable to get flow control status, terminal not connected.";
        return _xonXoff;
    }
}
...
char Pty::eraseChar() const
{
    if (pty()->masterFd() >= 0) {
        ...
        qCDebug(TerminalDebug) << "Unable to get erase char attribute, terminal not connected.";
        return _eraseChar;
    }
}
    ...
    if (QT_STAT(pty()->ttyName(), &sbuf) == 0) {
        if (writeable) {
            if (::chmod(pty()->ttyName(), sbuf.st_mode | S_IWGRP) < 0) {
                qCDebug(TerminalDebug) << "Could not set writeable on "<<pty()->ttyName();
            }
        } else {
            if (::chmod(pty()->ttyName(), sbuf.st_mode & ~(S_IWGRP | S_IWOTH)) < 0) {
                qCDebug(TerminalDebug) << "Could not unset writeable on "<<pty()->ttyName();
            }
        }
    } else {
        qCDebug(TerminalDebug) << "Could not stat "<<pty()->ttyName();
    }
}
...
{
    if (pty()->masterFd() < 0) {
        qCDebug(TerminalDebug) << "Unable to get eof char attribute, terminal not connected.";
        return;
    }
    struct ::termios ttyAttributes;
    pty()->tcGetAttr(&ttyAttributes);
    ...
    if (pty()->write(QByteArray(1, eofChar)) == -1) {
        qCDebug(TerminalDebug) << "Unable to send EOF";
    }
