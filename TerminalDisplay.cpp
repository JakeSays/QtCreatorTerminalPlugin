...
void TerminalDisplay::setScreenWindow(ScreenWindow* window)
{
    ...
    if (!_screenWindow.isNull()) {
        connect(_screenWindow.data() , &terminal::ScreenWindow::outputChanged , this , &terminal::TerminalDisplay::updateLineProperties);
        connect(_screenWindow.data() , &terminal::ScreenWindow::outputChanged , this , &terminal::TerminalDisplay::updateImage);
        connect(_screenWindow.data() , &terminal::ScreenWindow::currentResultLineChanged , this , &terminal::TerminalDisplay::updateImage);
        connect(_screenWindow.data(), &terminal::ScreenWindow::outputChanged, this, [this]() {
            _filterUpdateRequired = true;
        });
        connect(_screenWindow.data(), &terminal::ScreenWindow::scrolled, this, [this]() {
            _filterUpdateRequired = true;
        });
...
//    strategy |= QFont::ForceIntegerMetrics;
    ...
    // will cause terminal to crash later.
    QFontMetrics fontMetrics2(newFont);
    if (fontMetrics2.height() < 1) {
        qCDebug(TerminalDebug)<<"The font "<<newFont.toString()<<" has an invalid height()";
    ...
    // experimental optimization.  terminal assumes that the terminal is using a
    ...
    {
        const QString nonMatching = QString::asprintf("%s,%g,%d,%d,%d,%d,%d,%d,%d",
        ...
        qCDebug(TerminalDebug) << "The font to use in the terminal can not be matched exactly on your system.";
        qCDebug(TerminalDebug) << " Selected: " << newFont.toString();
        qCDebug(TerminalDebug) << " System  : " << nonMatching;
    }
...
void TerminalDisplay::resetFontSize()
{
    ...
    QFont font = getVTFont();
    const qreal defaultFontSize = _profile->font().pointSizeF();
...
TerminalDisplay::TerminalDisplay(Session* session, QWidget* parent)
    : QWidget(parent)
    ...
    , _filterUpdateRequired(true)
    , _fileFilter(new FileFilter(session))
    ...
    , _drawOverlay(false)
{
    ...
    connect(_scrollBar, &QScrollBar::sliderMoved, this, &terminal::TerminalDisplay::viewScrolledByUser);

    // setup timers for blinking text
    _blinkTextTimer = new QTimer(this);
    _blinkTextTimer->setInterval(TEXT_BLINK_DELAY);
    connect(_blinkTextTimer, &QTimer::timeout, this, &terminal::TerminalDisplay::blinkTextEvent);

    // setup timers for blinking cursor
    _blinkCursorTimer = new QTimer(this);
    _blinkCursorTimer->setInterval(QApplication::cursorFlashTime() / 2);
    connect(_blinkCursorTimer, &QTimer::timeout, this, &terminal::TerminalDisplay::blinkCursorEvent);
...
#ifndef QT_NO_ACCESSIBILITY
    QAccessible::installFactory(terminal::accessibleInterfaceFactory);
#endif
}
...
//    Q_ASSERT(!_sessionController->session().isNull());
    ...
    if (_profile != nullptr) {
        auto shape = static_cast<Enum::CursorShapeEnum>(_profile->property<int>(Profile::CursorShape));

        setKeyboardCursorShape(shape);
        setBlinkingCursorEnabled(_profile->blinkingCursorEnabled());
    }
}
...
void TerminalDisplay::showResizeNotification()
{
    if (_showTerminalSizeHint && isVisible()) {
        if (_resizeWidget == nullptr) {
            ...
            _resizeWidget->setMinimumWidth(_resizeWidget->fontMetrics().boundingRect(QString("Size: XXX x XXX")).width());
...
void TerminalDisplay::paintFilters(QPainter& painter)
{
    ...
    foreach(Filter::HotSpot* spot, spots) {
        ...
        for (int line = spot->startLine() ; line <= spot->endLine() ; line++) {
            ...
            } else if (spot->type() == SpotType::Marker) {
...
//            _dragInfo.state = diNone;
    ...
    //  select text or columnSelection
        ...
        if (!_usesMouseTracking || ((ev->modifiers() & Qt::ShiftModifier) != 0u)) {
            ...
            auto linkType = spot != nullptr
                ? spot->type()
                : SpotType::NotSpecified;
            emit configureRequest(ev->pos(), linkType);
        } else {
            if(!_readOnly) {
    ...
    for (const QChar &c : text) {
        if (!c.isPrint() && c != QLatin1Char('\t') && c != QLatin1Char('\n')) {
            ...
            case '\x1b':
                ...
                break;
            default:
                description = QString("Other unprintable character (\\x%1)").arg(c.unicode(), 0, 16);
                break;
            }
...
void TerminalDisplay::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    if (event->reason() != QContextMenuEvent::Mouse) {
        emit configureRequest(mapFromGlobal(QCursor::pos()), SpotType::NotSpecified);
    }
}
...
{
    swapFGBGColors();
    QTimer::singleShot(200, this, &terminal::TerminalDisplay::swapFGBGColors);
}
...
void TerminalDisplay::applyProfile(const Profile::Ptr &profile)
{
    ...
    {
        _filterChain->addFilter(_fileFilter);
    }
    else
    {
        _filterChain->removeFilter(_fileFilter);
