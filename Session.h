 ...
 * The pseudo-teletype (or PTY) handles I/O between the terminal process and terminal.
...
public Q_SLOTS:
     ...
     * the terminal connection (the pty) is closed and terminal waits for the
...
Q_SIGNALS:
    ...
    void receiveBlock(const char *buf, int len);
...
private:
    ...
    static QString checkShellProgram(const QString& program);
    QString locateShell();
