...
public:
    KAuthorizedPrivate()
        :   actionRestrictions(false), blockEverything(false), mutex()
    {
 ...
 * @private
 */
void allowUrlActionInternal(const QString &action, const QUrl &_baseURL, const QUrl &_destURL)
{
    MY_D
 ...
 * @private
 */
bool authorizeUrlActionInternal(const QString &action, const QUrl &_baseURL, const QUrl &_destURL, const QString &baseClass, const QString &destClass)
{
    MY_D
    QMutexLocker locker(&(d->mutex));
    if (d->blockEverything) {
